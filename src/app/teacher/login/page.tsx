...
export default function TeacherLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim()) {
      showToast('Email Required', 'Please enter your registered teacher/faculty email address.', 'error');
      return;
    }
    ...
    if (res.success && res.role === 'teacher') {
      showToast('Welcome Faculty Member', 'Entering Teacher Portal...', 'success');
      ...
      showToast('Not Found', res.message || 'No teacher/faculty account registered with this email.', 'error');
    }
  };
      ...
      <Card className="w-full max-w-md p-8 bg-white border border-neutral-200/80 shadow-lg rounded-2xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-xl font-bold tracking-tight text-neutral-900">
            Teacher & Faculty Sign In
          </h1>
          <p className="text-xs text-neutral-500">
            Access student rosters, team directories, direct contact channels, and Sangam documents.
          </p>
        </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <Input
                ...
                placeholder="e.g. prof.williams@university.edu"
          ...
          >
            {isLoading ? (
              <>
                ...
                <span>Verifying faculty records...</span>
              </>
            ) : (
        ...
        <div className="pt-3 border-t border-neutral-100 space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              type="button"
              onClick={() => setEmail('satya@gmail.com')}
              className="p-2 rounded-lg border border-neutral-200 hover:border-neutral-900 hover:bg-neutral-50 text-left transition-colors cursor-pointer"
            >
              <div className="text-[11px] font-bold text-neutral-800">SAtya</div>
              <div className="text-[10px] text-neutral-500 truncate">satya@gmail.com</div>
            </button>
            <button
              type="button"
              onClick={() => setEmail('teacher@sangamconnect.org')}
              className="p-2 rounded-lg border border-neutral-200 hover:border-neutral-900 hover:bg-neutral-50 text-left transition-colors cursor-pointer"
            >
              <div className="text-[11px] font-bold text-neutral-800">Faculty Lead</div>
              <div className="text-[10px] text-neutral-500 truncate">teacher@sangamconnect.org</div>
            </button>
          </div>
          <p className="text-[10px] text-neutral-400 text-center pt-1">
            Click any account above to auto-fill your credentials
          </p>
        </div>
      </Card>
    </div>
